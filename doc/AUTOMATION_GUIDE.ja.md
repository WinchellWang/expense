...
- iOS 17.0以降を搭載した **iPhone**（iOS 17.6+ / iOS 18+ 推奨）。
...
┌──────────────────────────────────────────────────────────┐
│  いずれかのカードをタップしたとき                           │
│  カテゴリ：飲食 (Food & Drinks)  |  加盟店：任意の加盟店    │
│  実行：すぐに実行                |  通知：オフ              │
└────────────────────────────┬─────────────────────────────┘
                             │
                             ▼
...
| トリガーのカテゴリ | Expense の対応カテゴリ | 主な利用シーンの例 |
| :--- | :--- | :--- |
| **飲食 (Food & Drinks)** | `🍱 飲食` | レストラン、カフェ、スーパー、デリバリー |
| **買い物 (Shopping)** | `🛍️ 買い物` | アパレル、雑貨、家電量販店 |
| **交通 (Transportation)** | `🚗 交通` | ガソリンスタンド、電車、タクシー、駐車場 |
| **旅行 (Travel)** | `🏖️ 旅行` | 航空券、ホテル、新幹線チケット |
| **サービス (Services)** | `🛠️ サービス` | 美容院、修理、クリーニング |
| **娯楽 (Entertainment)** | `🎠 娯楽` | 映画館、コンサート、アミューズメント施設 |
| **健康 (Health)** | `💊 健康` | ドラッグストア、クリニック、ジム会費 |

### 🌟 未分類防止のセーフティネット
追加で以下のオートメーションを1つ作成しておくことを推奨します：
- **カテゴリ**：**「任意のカテゴリ」**
- **Expense カテゴリ**：**`🏷️ 一般 (General)`**

> これにより、どのカテゴリにも当てはまらない支払いや例外的な取引も漏れなく記録できます！
